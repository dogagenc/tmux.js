...
describe("TmuxClient.killWindow", () => {
	it.effect("returns stdout", () =>
		Effect.gen(function* () {
			const tmux = yield* TmuxClient;
			expect(yield* tmux.killWindow()).toBe("");
		}).pipe(Effect.provide(tmuxFrom(empty))),
	);

	it.effect("killWindow() emits the bare subcommand", () =>
		Effect.gen(function* () {
			expect(yield* captureArgs((tmux) => tmux.killWindow())).toEqual([
				"kill-window",
			]);
		}),
	);

	it.effect("killWindow({ targetWindow }) emits -t", () =>
		Effect.gen(function* () {
			expect(
				yield* captureArgs((tmux) =>
					tmux.killWindow({ targetWindow: "work:1" }),
				),
			).toEqual(["kill-window", "-t", "work:1"]);
		}),
	);

	it.effect("killWindow emits the -a switch", () =>
		Effect.gen(function* () {
			expect(
				yield* captureArgs((tmux) =>
					tmux.killWindow({ killOthers: true, targetWindow: "work:1" }),
				),
			).toEqual(["kill-window", "-a", "-t", "work:1"]);
		}),
	);
});

describe("argument validation (killWindow)", () => {
		...
		Effect.gen(function* () {
			...
			yield* Effect.gen(function* () {
				...
				const error = yield* Effect.flip(
					tmux.killWindow({ targetWindow: "" as never }),
				);
