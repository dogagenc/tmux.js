...
describe("TmuxClient.unlinkWindow", () => {
	it.effect("resolves to an empty string on success", () =>
		Effect.gen(function* () {
			const tmux = yield* TmuxClient;
			expect(yield* tmux.unlinkWindow({ targetWindow: "other:2" })).toBe("");
		}).pipe(Effect.provide(tmuxFrom(empty))),
	);

	it.effect("emits the destroy and window flags", () =>
		Effect.gen(function* () {
			expect(
				yield* captureArgs((tmux) =>
					tmux.unlinkWindow({ destroy: true, targetWindow: "other:2" }),
				),
			).toEqual(["unlink-window", "-k", "-t", "other:2"]);
		}),
	);
});

describe("argument validation (unlinkWindow)", () => {
		...
		Effect.gen(function* () {
			...
			yield* Effect.gen(function* () {
				...
				const error = yield* Effect.flip(
					tmux.unlinkWindow({ targetWindow: "" as never }),
				);
