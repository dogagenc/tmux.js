...
describe("TmuxClient.selectWindow", () => {
	it.effect("resolves to an empty string on success", () =>
		Effect.gen(function* () {
			const tmux = yield* TmuxClient;
			expect(yield* tmux.selectWindow({ targetWindow: "work:1" })).toBe("");
		}).pipe(Effect.provide(tmuxFrom(empty))),
	);

	it.effect("emits the target window flag with its value", () =>
		Effect.gen(function* () {
			expect(
				yield* captureArgs((tmux) =>
					tmux.selectWindow({ targetWindow: "work:1" }),
				),
			).toEqual(["select-window", "-t", "work:1"]);
		}),
	);

	it.effect("emits the last, next, previous, and toggle flags", () =>
		Effect.gen(function* () {
			expect(
				yield* captureArgs((tmux) =>
					tmux.selectWindow({
			...
			).toEqual(["select-window", "-l", "-n", "-p", "-T"]);
		}),
	);
});

describe("argument validation (selectWindow)", () => {
		...
		Effect.gen(function* () {
			...
			yield* Effect.gen(function* () {
				...
				const error = yield* Effect.flip(
					tmux.selectWindow({ targetWindow: "" as never }),
				);
