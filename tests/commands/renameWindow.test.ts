import { describe, expect, it } from "@effect/vitest";
import { Effect } from "effect";
import { TmuxCommandOptionsError } from "../../src/Errors";
import { TmuxClient } from "../../src/exports/effect";
import { capturingTmux, tmuxFrom } from "../utils";

const empty = { stdout: "", stderr: "", exitCode: 0 };

const captureArgs = (
	run: (tmux: TmuxClient["Service"]) => Effect.Effect<unknown, unknown>,
) => {
	const harness = capturingTmux(empty);
	return Effect.gen(function* () {
		const tmux = yield* TmuxClient;
		yield* run(tmux);
		return harness.captured.args;
	}).pipe(Effect.provide(harness.layer));
};

describe("TmuxClient.renameWindow", () => {
	it.effect("returns stdout", () =>
		Effect.gen(function* () {
			const tmux = yield* TmuxClient;
			expect(yield* tmux.renameWindow("editor")).toBe("");
		}).pipe(Effect.provide(tmuxFrom(empty))),
	);

	it.effect("renameWindow(newName) emits positional new name", () =>
		Effect.gen(function* () {
			expect(yield* captureArgs((tmux) => tmux.renameWindow("editor"))).toEqual(
				["rename-window", "editor"],
			);
		}),
	);

	it.effect(
		"renameWindow(newName, { targetWindow }) emits -t before new name",
		() =>
			Effect.gen(function* () {
				expect(
					yield* captureArgs((tmux) =>
						tmux.renameWindow("editor", { targetWindow: "0" }),
					),
				).toEqual(["rename-window", "-t", "0", "editor"]);
			}),
	);
});

describe("argument validation (renameWindow)", () => {
	it.effect("rejects an empty new name before spawning tmux", () =>
		Effect.gen(function* () {
			const harness = capturingTmux(empty);
			yield* Effect.gen(function* () {
				const tmux = yield* TmuxClient;
				const error = yield* Effect.flip(tmux.renameWindow("" as never));
				expect(error).toBeInstanceOf(TmuxCommandOptionsError);
				expect(harness.captured.args).toEqual([]);
			}).pipe(Effect.provide(harness.layer));
		}),
	);
});
