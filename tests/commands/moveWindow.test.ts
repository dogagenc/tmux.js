...
describe("TmuxClient.moveWindow", () => {
	...
	it.effect("emits the before flag", () =>
		Effect.gen(function* () {
			expect(
				yield* captureArgs((tmux) =>
					tmux.moveWindow({ before: true, targetWindow: "other:2" }),
				),
			).toEqual(["move-window", "-b", "-t", "other:2"]);
		}),
	);

	it.effect("emits the renumber flag", () =>
		Effect.gen(function* () {
			expect(
				yield* captureArgs((tmux) =>
					tmux.moveWindow({ renumber: true, sourceWindow: "work" }),
				),
			).toEqual(["move-window", "-r", "-s", "work"]);
		}),
	);
});

describe("argument validation (moveWindow)", () => {
		...
		Effect.gen(function* () {
			...
			yield* Effect.gen(function* () {
				...
				const error = yield* Effect.flip(
					tmux.moveWindow({ targetWindow: "" as never }),
				);
