...
describe("TmuxClient.previousLayout", () => {
	it.effect("returns stdout", () =>
		Effect.gen(function* () {
			const tmux = yield* TmuxClient;
			expect(yield* tmux.previousLayout()).toBe("");
		}).pipe(Effect.provide(tmuxFrom(empty))),
	);

	it.effect("previousLayout() emits the bare subcommand", () =>
		Effect.gen(function* () {
			expect(yield* captureArgs((tmux) => tmux.previousLayout())).toEqual([
				"previous-layout",
			]);
		}),
	);

	it.effect("previousLayout({ targetWindow }) emits -t", () =>
		Effect.gen(function* () {
			expect(
				yield* captureArgs((tmux) =>
					tmux.previousLayout({ targetWindow: "work:1" }),
				),
			).toEqual(["previous-layout", "-t", "work:1"]);
		}),
	);
});

describe("argument validation (previousLayout)", () => {
		...
		Effect.gen(function* () {
			...
			yield* Effect.gen(function* () {
				...
				const error = yield* Effect.flip(
					tmux.previousLayout({ targetWindow: "" as never }),
				);
