...
describe("TmuxClient.lastWindow", () => {
	it.effect("returns stdout", () =>
		Effect.gen(function* () {
			const tmux = yield* TmuxClient;
			expect(yield* tmux.lastWindow()).toBe("");
		}).pipe(Effect.provide(tmuxFrom(empty))),
	);

	it.effect("lastWindow() emits the bare subcommand", () =>
		Effect.gen(function* () {
			expect(yield* captureArgs((tmux) => tmux.lastWindow())).toEqual([
				"last-window",
			]);
		}),
	);

	it.effect("lastWindow({ targetSession }) emits -t", () =>
		Effect.gen(function* () {
			expect(
				yield* captureArgs((tmux) =>
					tmux.lastWindow({ targetSession: "work" }),
				),
			).toEqual(["last-window", "-t", "work"]);
		}),
	);
});

describe("argument validation (lastWindow)", () => {
		...
		Effect.gen(function* () {
			...
			yield* Effect.gen(function* () {
				...
				const error = yield* Effect.flip(
					tmux.lastWindow({ targetSession: "" as never }),
				);
