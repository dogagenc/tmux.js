...
describe("TmuxClient.swapWindow", () => {
	it.effect("resolves to an empty string on success", () =>
		Effect.gen(function* () {
			const tmux = yield* TmuxClient;
			expect(yield* tmux.swapWindow({ targetWindow: "work:1" })).toBe("");
		}).pipe(Effect.provide(tmuxFrom(empty))),
	);

	it.effect("emits the detached flag and valued flags", () =>
		Effect.gen(function* () {
			expect(
			...
			).toEqual(["swap-window", "-d", "-s", "work:1", "-t", "work:2"]);
		}),
	);
});

describe("argument validation (swapWindow)", () => {
		...
		Effect.gen(function* () {
			...
			yield* Effect.gen(function* () {
				...
				const error = yield* Effect.flip(
					tmux.swapWindow({ targetWindow: "" as never }),
				);
